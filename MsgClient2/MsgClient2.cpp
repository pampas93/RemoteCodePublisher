...
#include "MsgClient2.h"
#include <shellapi.h>
#pragma comment(lib,"shell32.lib")
size_t ClientCounter2::clientCount = 0;
...
*/
HttpMessage MsgClient2::makeMessage(size_t n, const std::string& body, const EndPoint& ep, std::string category, std::string type)
{
	HttpMessage msg;
	HttpMessage::Attribute attrib;
	EndPoint myEndPoint = "localhost:8084";
	switch (n)
	{
	case 1:
		msg.clear();
...
void MsgClient2::sendMessage(HttpMessage& msg, Socket& socket)
{
...
bool MsgClient2::sendFile(const std::string& filenameAbs, Socket& socket, std::string category)
{
...
void MsgClient2::execute(const size_t TimeBetweenMessages, const size_t NumMessages, std::string type, std::string category, std::string path)
{
...
void MsgClient2::sendFiles2Server(Socket & socket, std::string category)
{
...
HttpMessage ServerHandler2::readMessage(Socket& socket)
{
...
bool ServerHandler2::readFile(const std::string& filename, size_t fileSize, Socket& socket, std::string category)
{
...
void ServerHandler2::operator()(Socket socket)
{
	while (true)
	{
...
HttpMessage MsgClient2::listenerFunction()
{
...
bool MsgClient2::analyzeMessageFromServer(HttpMessage m)
{
...
std::string MsgClient2::getCategory(int c) {
	switch (c) {
	case 1:
		return "category1";
		break;
...
std::string MsgClient2::uploadFunction(std::string path, int cat, std::vector<std::string> files)
{
	filestoOpen = files;
...
std::string MsgClient2::deleteFunction(int cat)
{
	std::string category = getCategory(cat);
	std::thread t1(
...
std::string MsgClient2::publishFunction(int cat)
{
	std::string category = getCategory(cat);
	std::thread t1(
...
std::string MsgClient2::displayFunction(int cat)
{
	std::string category = getCategory(cat);
	std::thread t1(
...
std::string MsgClient2::downloadFunction(int cat, std::vector<std::string> filesToOpen)
{
	std::string category = getCategory(cat);
	std::thread t1(
...
std::string MsgClient2::downloadLazyFunction(int cat, std::string file)
{
	lazyFile = file;
	std::string category = getCategory(cat);
	std::thread t1(
...
void MsgClient2::openBrowser(std::string fpath)
{
...
int main()
{
	...
	//c.deleteFunction(2);
	::Sleep(10000);
	//c.uploadFunction("../TestFiles", 3);
	//c.publishFunction(1);
	//c.displayFunction(1);
	//c.downloadLazyFunction(1, "XmlDocument.h.html");
	//c.deleteFunction(1);
	//c.displayFunction(1);
